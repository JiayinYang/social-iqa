...
class SocialIQaProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        kbpath = os.path.join(data_dir, "rerank_data_train.pkl")
    ...
    def get_dev_examples(self, data_dir):
        ...
        with open(newpath, 'r', encoding='utf-8') as f:
            data = f.readlines()
        kbpath = os.path.join(data_dir, "rerank_data_dev.pkl")
        kb_data = pickle.load( open(kbpath, "rb" ) )
    ...
    def get_test_examples(self, data_dir):
        ...
        with open(newpath, 'r', encoding='utf-8') as f:
            data = f.readlines()
        kbpath = os.path.join(data_dir, "rerank_data_test.pkl")
        kb_data = pickle.load( open(kbpath, "rb" ) )
    ...
    def _create_examples(self, lines, kb_data, set_type):
        ...
        for idx, line in enumerate(lines):
            ...
            context_a = ""
            for l in context_a_list:
                context_a += l.replace("\n",". ")
            context_a = context_a[:-1]

            context_b = ""
            for l in context_b_list:
                context_b += l.replace("\n",". ")
            context_b = context_b[:-1]

            context_c = ""
            for l in context_c_list:
                context_c += l.replace("\n",". ")
...
processors = {"socialiqa": SocialIQaProcessor}
